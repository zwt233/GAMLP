...
import uuid
import random
from utils import gen_output,set_seed,train,val,test
import uuid
...
def run(args, device):
    ...
    for stage,epochs in enumerate(args.stages):    
        ...
        if stage==0:
            ...
            train_loader = torch.utils.data.DataLoader(
                        torch.arange(len(train_nid)), batch_size=int(args.batch_size*len(train_nid)/(len(enhance_idx)+len(train_nid))), shuffle=True, drop_last=False)            
        ...
        for stage,epoch in enumerate(args.stages):
            ...
            preds=gen_output(model,feats,all_loader,labels.device)
